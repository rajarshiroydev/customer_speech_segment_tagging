...
approaches/            Five experiment notebooks and their outputs/evaluations
...
```text
conversation_candidates.csv           Candidate speech/conversation blocks
final_two_conversations.csv           Final two selected customer conversations
tagged_conversation_candidates.csv    Candidates annotated with selected customer assignment
...
Higher `IoU` is better. Lower boundary error is better. I used mean IoU as the main ranking metric because the task involves interval detection along with point-boundary detection.

## Approaches

| Approach | Method | Pros | Cons |
|---|---|---|---|
| Approach 1: Energy Baseline | Uses frame-level energy/amplitude to find active audio regions, merges nearby regions, and selects likely conversations. | Very fast, easy to explain, no model dependencies, useful as a sanity-check baseline. | Sensitive to background shop noise, misses quiet speech, cannot distinguish customer speech from staff speech, weak semantic understanding. |
| Approach 2: VAD Only | Uses Silero VAD to detect speech segments, merges them into conversation candidates, and selects final intervals. | More robust than raw energy, multilingual speech detection works reasonably well, still lightweight and explainable. | Detects all speech, including staff chatter; long pauses split real conversations; no speaker or semantic understanding. |
...
| 3 | Approach 2: VAD Only | 0.250 | 118.969 | 184.468 | 151.719 |
| 4 | Approach 1: Energy Baseline | 0.250 | 119.000 | 184.467 | 151.733 |
...
Approaches 1 and 2 mostly select high-activity speech regions. They are useful baselines, but they fail when real customer conversations contain pauses or when non-customer speech is acoustically similar to customer speech.

Approach 3 adds diarization, but diarization alone does not solve the task. It identifies speaker clusters, not customer identities. In noisy multilingual retail audio, speaker labels can be fragmented or inconsistent, which caused poor end-boundary estimates.
